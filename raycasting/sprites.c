...
void	draw_sprite(t_data *data, int stripe, int text_x, int text_y)
{
	...
	while (++i < data->sprite.draw_end_y)
	{
		...
		if (data->sprite.img.addr[4 * text_y * data->sprite.img.height + 4 *
				text_x]) // si c'est transparent
			data->img.addr[4 * stripe + 4 * data->param.resolution.axe_x * i] =
				data->sprite.img.addr[4 * text_y * data->sprite.img.height + 4
				* text_x];
		if (data->sprite.img.addr[text_y * data->sprite.img.height * 4 + text_x
				* 4 + 1]) // si c'est rouge
			data->img.addr[4 * stripe + 4 * data->param.resolution.axe_x * i
				+ 1] = data->sprite.img.addr[text_y * data->sprite.img.height *
				4 + text_x * 4 + 1];
		if (data->sprite.img.addr[text_y * data->sprite.img.height * 4 + text_x
				* 4 + 2]) // si c'est vert
			data->img.addr[4 * stripe + 4 * data->param.resolution.axe_x * i +
				2] = data->sprite.img.addr[text_y * data->sprite.img.height * 4
				+ text_x * 4 + 2];
		if (data->sprite.img.addr[text_y * data->sprite.img.height * 4 + text_x
				* 4 + 3]) // si c'est bleu
			data->img.addr[4 * stripe + 4 * data->param.resolution.axe_x * i +
				3] = data->sprite.img.addr[text_y * data->sprite.img.height * 4
				+ text_x * 4 + 3];
	}
}
